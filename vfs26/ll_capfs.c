/*
 * Forward ported to Kernel 2.6 by
 * Murali Vilayannur (C) 2005
 * vilayann@cse.psu.edu
 *
 * original copyright (c) 1999 Rob Ross and Phil Carns, all rights reserved.
 *
...
capfs_size_t len, struct capfs_meta *meta, int special_op)
{
	...
	PDEBUG(D_LLEV, "ll_capfs_lookup called for %s [flags: %x]\n", name, sb->flags);
	if (special_op) {
...
int ll_capfs_rename(struct capfs_inode *old_inode, const capfs_char_t *new_name, capfs_size_t len)
{
	...
	strncpy(up.u.rename.new_name, new_name, CAPFSNAMELEN);
		...
		struct capfs_inode *old_inode, struct capfs_meta *meta)
{
	...
	init_structs(&up, &down, LINK_OP, use_tcp, cons);
	if(len > CAPFSNAMELEN) {
		return -ENAMETOOLONG;
	}
	up.u.link.meta = *meta;
	strncpy(up.v1.fhname, link_name, CAPFSNAMELEN);
	strncpy(up.u.link.target_name, old_inode->name, CAPFSNAMELEN);

	if ((error = capfsdev_enqueue(&up, &down)) < 0) {
		PERROR("ll_capfs_link failed on enqueue for %s->%s\n", link_name, old_inode->name);
		return error;
	}
		...
		const capfs_char_t* name, capfs_size_t len, struct capfs_meta *meta)
{
	...
	if(len > CAPFSNAMELEN || symlink_len > CAPFSNAMELEN) {
		return -ENAMETOOLONG;
	}
	/* create a soft link */
	up.u.symlink.meta = *meta;
	strncpy(up.v1.fhname, symlink_name, CAPFSNAMELEN);
 ...
 */
int ll_capfs_readlink(struct capfs_inode *new_inode, char *buffer, unsigned int bufbase, unsigned int buflen)
{
	int error;
	struct capfs_upcall up;
	struct capfs_downcall down;
	...
	/* describe buffer */
	up.xfer.to_kmem = 1;
	up.xfer.ptr = buffer + bufbase; /* note that this is a kernel space pointer */
	up.xfer.size = buflen;
