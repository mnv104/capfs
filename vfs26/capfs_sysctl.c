			     ...
			     void __user * buffer, size_t * lenp, loff_t *ppos)
{
	if ( write ) {
		...
		*lenp = capfs_vfs_stats_get_info(buffer, NULL, *ppos, *lenp);
	}
	*ppos += *lenp;
	return 0;
}
				...
				size_t * lenp, loff_t *ppos)
{
	if ( write ) {
		...
		*lenp = capfs_upcall_stats_get_info(buffer, NULL, *ppos, *lenp);
				    ...
				    size_t * lenp, loff_t *ppos)
{
	if ( write ) {
		...
		*lenp = capfs_rwop_stats_get_info(buffer, NULL, *ppos, *lenp);
	}
	*ppos += *lenp;
	return 0;
}
				    ...
				    size_t * lenp, loff_t *ppos)
{
	/* only reads allowed */
	if ( !write ) {
		*lenp = capfs_capfsd_stats_get_info(buffer, NULL, *ppos, *lenp);
	}
	if (*lenp >= 0)
	{
		*ppos += *lenp;
...
int capfs_capfsd_stats_get_info(char __user * buffer, char ** start, off_t offset,
			     int length)
{
	int len, i;
...
int capfs_capfsd_stats_get_info_kern(char * buffer, char ** start, off_t offset,
			     int length)
{
	int i, len;
	const int LIMIT = PAGE_SIZE - 80;/* Let us not take any chances here */
	...
	/* First issue an upcall to the daemon and get the stats */
	if ((len = ll_capfs_hint(NULL, HINT_STATS, &cstats)) < 0) {
		PERROR("ll_capfs_hint[HINT_STATS failed]\n");
		return len;
	}
	len = 0;
	do {
		...
		if(len >=  LIMIT) break;
		len += sprintf(buffer + len, "%-20s %9lld\n", "Hcache hits:", cstats.hcache_hits);
		if(len >=  LIMIT) break;
		len += sprintf(buffer + len, "%-20s %9lld\n", "Hcache misses:", cstats.hcache_misses);
		if(len >=  LIMIT) break;
		len += sprintf(buffer + len, "%-20s %9lld\n", "Hcache fetches:", cstats.hcache_fetches);
		...
		for (i = 0; i < CAPFS_STATS_MAX; i++)
		{
			...
			{
				if(len >=  LIMIT) break;
				len += sprintf(buffer + len, "%-20s %d %9lld\n", "Get time on CAS ", i, cstats.server_get_time[i]);
			}
		}
		for (i = 0; i < CAPFS_STATS_MAX; i++)
		{
...
int capfs_vfs_stats_get_info(char __user * buffer, char ** start, off_t offset,
			     int length)
{
...
int capfs_vfs_stats_get_info_kern(char * buffer, char ** start, off_t offset,
			     int length)
{
	struct capfs_vfs_stats * ps = &capfs_vfs_stat;
	int len;
...
int capfs_upcall_stats_get_info_kern(char * buffer, char ** start, off_t offset, int length)
{
	int i,len;
	off_t begin;
	const int LIMIT = PAGE_SIZE - 80;/* Let us not take any chances here */
...
int capfs_rwop_stats_get_info_kern(char * buffer, char ** start, off_t offset, int length)
{
	int i, len;
	off_t begin;
	/* There can only be 10 buckets, so we have to have a buffer of length 40
...
void capfs_sysctl_init()
{
	...
	if (proc_fs_capfs) {
		...
		if((entry = capfs_proc_create("vfs_stats", capfs_vfs_stats_get_info_kern)) == NULL) {
			printk(KERN_INFO "Could not create vfs_stats in proc\n");
		}
		if((entry = capfs_proc_create("upcall_stats", capfs_upcall_stats_get_info_kern)) == NULL) {
			printk(KERN_INFO "Could not create upcall_stats in proc\n");
		}
		if((entry = capfs_proc_create("rw_stats", capfs_rwop_stats_get_info_kern)) == NULL) {
			printk(KERN_INFO "Could not creat rw_stats in proc\n");
		}
		if((entry = capfs_proc_create("capfsd_stats", capfs_capfsd_stats_get_info_kern)) == NULL) {
