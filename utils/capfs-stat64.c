...
/* looks a lot like capfs_stat.c, no? well it is.  just changing a 
 * datatype or two to handle 64 bits */
...
int main(int argc, char **argv)
{
	...
	printf("recompile with _LARGEFILE64_SOURCE defined\n");
#else
	struct stat64 sbuf;

	if (argc <= 1 || capfs_stat64(argv[1], &sbuf) < 0) exit(1);

	printf("%s:\n", argv[1]);
	printf("st_dev = %d, st_ino = %d, st_mode = %o, st_nlink = %d\n",
	(int) sbuf.st_dev, (int) sbuf.st_ino, (int) sbuf.st_mode, (int) sbuf.st_nlink);
	printf("st_uid = %d, st_gid = %d, st_rdev = %d, st_size = %lld\n",
	(int) sbuf.st_uid, (int) sbuf.st_gid, (int) sbuf.st_rdev, ( long long )sbuf.st_size);
