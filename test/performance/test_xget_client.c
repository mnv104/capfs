...
int main(int argc, char *argv[])
{
	...
	char chunkName[1024], *recipie, *readPtr;
...
use_sockets:0,
	...
	while ((bytesRead = getline(&line, &len, cFile)) != -1) {
		...
		recipie=malloc(fSize);
		read(recipieFd, recipie, fSize);
		readPtr=recipie;
		...
		{
			/*create the jobs*/
			jobs[i].hashes=&(recipie[CAPFS_MAXHASHLENGTH * hashesUsed]);
			...
			hashesUsed+=cr_workerData->count;
		...
		clnt_get(1, jobs, iodsUsed);
		/*
		for(i=0;i<iodsUsed;i++)
		{
			unsigned char *hptr=NULL, *indexPtr;
			int digestLen;
			jobPtr=&(jobs[i]);
			indexPtr=jobPtr->hashes;
			for(j=0;j<jobPtr->data->count;j++)
			{
				hptr=digest(jobPtr->data->buf[j].start,jobPtr->data->buf[j].byteCount);
				if (memcmp(hptr,indexPtr,CAPFS_MAXHASHLENGTH)!=0)
					printf("bad fetch\n");
				indexPtr+=CAPFS_MAXHASHLENGTH;
				free(hptr);hptr=NULL;
			}
