...
int register_svc(int *sockp, int proto, int *port, int *prog_num, int version)
{
	...
	if (proto != IPPROTO_TCP && proto != IPPROTO_UDP) {
		LOG(stderr, CRITICAL_MSG, SUBSYS_SHARED, "Invalid protocol type specified\n");
		errno = EINVAL;
		return -1;
	}
	...
	if (*sockp == RPC_ANYSOCK) {
		...
		if (s < 0) {
			LOG(stderr, CRITICAL_MSG, SUBSYS_SHARED, "Socket create failure\n");
			*sockp = -1;
			return -1;
		}
	}
	...
	if (proto == IPPROTO_TCP)
	{
		...
		{
			LOG(stderr, CRITICAL_MSG, SUBSYS_SHARED, "set_tcpopt on socket %d to kill Nagle failed %s\n",
					s, strerror(errno));
		}
	}
	...
	if (bind(s, (struct sockaddr *) &addr, len) != 0) {
		LOG(stderr, CRITICAL_MSG, SUBSYS_SHARED, "Bind to the specified port and/or ephemeral ports failed! %s\n", strerror(errno));
		if (*sockp == RPC_ANYSOCK) {
			close(s);
			*sockp = -1;
		}
		...
		struct sockaddr *my_address)
{
	...
	if (protocol == IPPROTO_UDP) {
		...
		{
			LOG(stderr, CRITICAL_MSG, SUBSYS_SHARED, "set_tcpopt on socket %d to kill Nagle's algorithm failed %s\n",
					s, strerror(errno));
		}
	}
...
int get_program_info(char *hostname, int protocol, int _port, int *version)
{
	...
	*version = -1;
	...
	if ((list = pmap_getmaps(&addr)) == NULL) {
		LOG(stderr, CRITICAL_MSG, SUBSYS_SHARED, "Could not get program number for the specified host\n");
		return -1;
	}
	do {
		...
		if (map->pm_prot == protocol && map->pm_port == port) {
			if (flag == 0) {
				...
				*version = map->pm_vers;
				...
				LOG(stderr, CRITICAL_MSG, SUBSYS_SHARED, "Can we have different program numbers/version for a specific host/port/proto combination\n");
				return -1;
			}
		}
...
static void key_create(void)
{
	...
	*/
	...
	if (sigaction(SIGUSR1, &handler, NULL) != 0) {
		LOG(stderr, CRITICAL_MSG, SUBSYS_SHARED, "Could not setup signal handler for SIGUSR1: %s\n", strerror(errno));
		exit(1);
	}
	return;
}
...
static int do_register(int proto, struct t_args *targs)
{
	...
	if (register_svc(&s, proto, &port, &prog_number, version_number) < 0) {
		LOG(stderr, CRITICAL_MSG, SUBSYS_SHARED, "Could not register with portmapper daemon!\n");
		targs->ta_tid = 0;
		targs->ta_status = -errno;
		return -1;
	}
	LOG(stderr, INFO_MSG, SUBSYS_SHARED, "Registered %s transport program number 0x%x "
	...
	if (proto == IPPROTO_TCP) {
		...
		if ((xprt = svctcp_create(s, 0, 0)) == NULL) {
			LOG(stderr, CRITICAL_MSG, SUBSYS_SHARED, "Could not setup TCP xport handle: %s\n",
					strerror(errno));
			targs->ta_tid = 0;
			targs->ta_status = -errno;
		...
		if ((xprt = svcudp_create(s)) == NULL) {
			LOG(stderr, CRITICAL_MSG, SUBSYS_SHARED, "Could not setup UDP xprt handle: %s\n", 
					strerror(errno));
			targs->ta_tid = 0;
			targs->ta_status = -errno;
			/* Deregister from the portmapper */
	...
	if (svc_register(xprt, prog_number, version_number, dispatch_fn, 0) == 0) {
		LOG(stderr, CRITICAL_MSG, SUBSYS_SHARED, "Could not register protocol %s service's dispatch function: %s\n", 
...
svc_func(void *_args)
{
	...
	if (pthread_setspecific(key, targs) != 0) {
		LOG(stderr, CRITICAL_MSG, SUBSYS_SHARED, "Could not set thread specific data\n");
		targs->ta_tid = 0;
		targs->ta_status = -errno;
	...
	if(pthread_sigmask(SIG_SETMASK, &set, NULL) != 0) {
		LOG(stderr, CRITICAL_MSG, SUBSYS_SHARED, "Could not set thread's signal mask\n");
		targs->ta_tid = 0;
		targs->ta_status = -errno;
		/* wake up your parent */
		sem_post(&targs->ta_sem);
	...
	svc_run();
	LOG(stderr, CRITICAL_MSG, SUBSYS_SHARED, "svc_run() should not return!\n");
	return NULL;
}
		...
		void (*handle_fn)(struct svc_req *, SVCXPRT *), struct svc_info *info)
{
	...
	if (handle_fn == NULL || info == NULL) {
		LOG(stderr, CRITICAL_MSG, SUBSYS_SHARED, "Invalid address specified for tid"
				"and/or dispatch function and/or svc_info pointer\n");
		return -EFAULT;
	}
	if (vers < 0) {
		LOG(stderr, CRITICAL_MSG, SUBSYS_SHARED, "-ve version number is not allowed!\n");
		return -EINVAL;
	}
	if (proto > 0 && (proto != IPPROTO_TCP && proto != IPPROTO_UDP)) {
		LOG(stderr, CRITICAL_MSG, SUBSYS_SHARED, "Invalid/unhandled value of protocol %d\n", proto);
		return -EINVAL;
	}
	if (info->use_thread != 0 && info->use_thread != 1) {
		LOG(stderr, CRITICAL_MSG, SUBSYS_SHARED, "Invalid value of use_thread = %d\n", info->use_thread);
		return -EINVAL;
	}
	...
	if (info->use_thread == 1) {
		...
		if ((error = pthread_create(&info->tid, NULL, svc_func, args)) != 0) {
			LOG(stderr, CRITICAL_MSG, SUBSYS_SHARED, "Could not spawn service thread!\n");
			free(args);
			return -error;
		}
		...
		if (args->ta_status != 0) {
			...
			free(args);
			LOG(stderr, CRITICAL_MSG, SUBSYS_SHARED, "Could not setup RPC service thread\n");
			return error;
		}
	...
	/* do not spawn a thread. Instead setup the RPC service and start waiting for them */
		...
		if (proto == -1 || proto == IPPROTO_TCP) {
			if (do_register(IPPROTO_TCP, args) < 0) {
				error = args->ta_status;
				LOG(stderr, CRITICAL_MSG, SUBSYS_SHARED, "Could not setup RPC service\n");
				free(args);
				return error;
			}
		}
		...
		svc_run();
		LOG(stderr, CRITICAL_MSG, SUBSYS_SHARED, "Critical! svc_run() should never return\n");
	}
	return 0;
}
