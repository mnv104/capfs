...
static ssize_t capfsdev_read(struct file *my_file, char *buf, 
								size_t buf_sz, loff_t *offset);
static ssize_t capfsdev_write(struct file *my_file, const char *buf, 
								 size_t buf_sz, loff_t *offset);
...
static inline int capfsdev_read_in_data(void *kptr, char *uptr, 
													capfs_size_t size);
static inline int capfsdev_write_out_data(char *uptr, void *kptr, 
													  capfs_size_t size);
static inline int check_for_bad_signals(int *sig_nr);
static inline void cleanup_buffer(struct capfs_upcall *in_upcall,
...
#ifdef HAVE_DEVFS
	if ((ret = devfs_register_chrdev(capfs_major, "capfsd", &capfsdev_fops)) < 0)
#else
...
#ifdef HAVE_DEVFS
	devfs_handle = devfs_register(NULL, "capfsd", DEVFS_FL_DEFAULT,
	capfs_major, 0, (S_IFCHR | S_IRUSR | S_IWUSR), &capfsdev_fops, NULL);
#endif
...
void capfsdev_cleanup(void)
{
   ...
   capfs_sysctl_clean();
	...
	devfs_unregister_chrdev(capfs_major, "capfsd");
#else
	...
	size_t buf_sz, loff_t *offset)
{
	...
	if (in_req.type == READ_OP) 
	{
		total_rd_time += in_req.total_time;
	}
	else if (in_req.type == WRITE_OP)
	{
	...
	switch (in_req.type) {
		case WRITE_OP:
          ...
          */
			if (!my_seq->step) {
				...
				PDEBUG(D_SPECIAL, "capfsdev: preparing to copy %ld bytes.\n", 
				...
				if (ret != 0) {
					up(&seq_sem);
					PDEBUG(D_SPECIAL, "capfsdev: capfs memory transfer failure.\n");
					return -ENOMEM;
				}
		...
		case READ_OP:
			...
			if (ret != 0) {
				up(&seq_sem);
				PDEBUG(D_SPECIAL,
				"capfsdev: copy_from_user/capfs_copy_to_userbuf failure.\n");
				return -ENOMEM;
			}
			break;
		case GETDENTS_OP:
 ...
 *
 * Parameters:
 * kptr - kernel space destination, could be an address or a kiobuf
													...
													capfs_size_t size)
{
	...
	if (capfs_buffer == CAPFS_BUFFER_MAPPED) {
      /* in this case we are copying from one userspace to another */
		ret = capfs_copy_to_userbuf((struct kiobuf *)kptr, uptr, size);
		return ret;
	}
 ...
 * kptr - kernel space source, could be an address or a kiobuf
 * size - size of data to move
 */
static inline int capfsdev_write_out_data(char *uptr, void *kptr, 
													  capfs_size_t size)
{
	...
	if (capfs_buffer == CAPFS_BUFFER_MAPPED) {
      /* in this case we are copying from one userspace to another */
		ret = capfs_copy_from_userbuf(uptr, (struct kiobuf *)kptr, size);
		return ret;
	}
 ...
 * Returns 0 on success, -errno on failure.
 */
static ssize_t capfsdev_read(struct file *my_file, char *buf, size_t buf_sz,
loff_t *offset)
{
	...
	if (up_pending.prev != &up_pending) {
		...
		spin_unlock(&capfsdev_up_lock);
		copy_to_user(buf, &(out_entry->pending_up), sizeof(struct capfs_upcall));
		kfree(out_entry);
...
static unsigned int capfsdev_poll(struct file *my_file, struct 
	...
	unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd){
		...
		case(TEST_RD_XFER):
			...
			copy_from_user(&rd_xfer_reg, (struct ioctl_mem_reg*)arg, sizeof(struct
				ioctl_mem_reg));
		...
		case INVALIDATE_CACHE:
		{
			...
			if (copy_from_user(&karg, args, sizeof(struct capfs_invalidate_ioctl)) < 0) {
				return -EFAULT;
			}
...
#ifdef HAVE_SIGHAND_IN_TASK_STRUCT
