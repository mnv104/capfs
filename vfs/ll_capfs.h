...
struct capfs_inode {
	struct capfs_rw_semaphore lock;
	capfs_handle_t handle;
	capfs_char_t *name;
	struct capfs_super *super;
};

static __inline__ int read_lock_inode(struct capfs_inode *pinode)
{
#ifdef  CAPFS_SERIALIZE_DCACHE
		  if(pinode) {
					 /* printk("read_lock: pinode: %p, handle: %ld\n",pinode, (unsigned long)pinode->handle); */
					 return Down_read_interruptible(&pinode->lock);
		  }
		  return -EINVAL;
#else
		  return 0;
...
#ifdef  CAPFS_SERIALIZE_DCACHE
		  if(pinode) {
					 /* printk("write_lock: pinode: %p, handle: %ld\n",pinode, (unsigned long)pinode->handle); */
					 return Down_write_interruptible(&pinode->lock);
		  }
		  return -EINVAL;
#else
		  return 0;
...
#ifdef  CAPFS_SERIALIZE_DCACHE
		  if(pinode) {
					 /* printk("read_unlock: pinode: %p, handle: %ld\n",pinode, (unsigned long)pinode->handle); */
					 Up_read(&pinode->lock);
					 return 0;
		  }
		  return -EINVAL;
...
#ifdef  CAPFS_SERIALIZE_DCACHE
		  if(pinode) {
					 /* printk("write_unlock: pinode: %p, handle: %ld\n",pinode,(unsigned long)pinode->handle);*/
					 Up_write(&pinode->lock);
					 return 0;
		  }
		  return -EINVAL;
#else
...
struct capfs_stats {
	...
	int64_t    sha1_time;
	/* Time spent in get RPC, put RPC and gethashes RPC */
	int64_t    rpc_get, rpc_put, rpc_gethashes, rpc_compute;
	/* Time spent in committing writes alone */
	int64_t    rpc_commit;
	/* Time spent on IO servers. currently we limit this to 16! */
...
int ll_capfs_rename(struct capfs_inode *old_inode, struct capfs_inode
	*new_inode);
int ll_capfs_lookup(struct capfs_super *sb, const capfs_char_t *name,
capfs_size_t len, struct capfs_meta *meta, int special_op);
int ll_capfs_link(struct capfs_inode* new_inode, struct capfs_inode *old_inode, struct capfs_meta *);
int ll_capfs_symlink(int use_tcp, int cons,
		struct capfs_inode* new_inode, const capfs_char_t* name, capfs_size_t len, struct capfs_meta *);
int ll_capfs_readlink(struct capfs_inode *new_inode, char*, unsigned int);
