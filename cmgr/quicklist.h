...
static __inline__ void qlist_add(struct qlist_head *new, struct qlist_head *head)
{
	...
	if (new->next || new->prev) {
		panic("qlist_add refusing to add! Found non NULL next\
				& prev pointers\n");
		exit(1);
	}
#endif
...
#if 0
	if (new->next || new->prev) {
		panic("qlist_add_tail refusing to add! Found non NULL next\
				& prev pointers\n");
		exit(1);
	}
#endif
...
static __inline__ void qlist_del(struct qlist_head *entry)
{
	...
	if (!entry->next || !entry->prev) {
		panic("qlist_del refusing to delete. Found NULL \
			next & prev pointers\n");
		exit(1);
	}
#endif
